...
class EmailVerifierService:
    ...
    # def verify_emails_for_campaign(self, campaign_id: str) -> int:
        ...
        try:
            ...
            app_logger.info(f"Found {len(leads)} leads to verify for campaign {campaign_id}", extra={'component': 'server'})
            ...
            for lead in leads:
                try:
                    ...
                    if verified_count % 10 == 0:
                        app_logger.info(f"Verified {verified_count}/{len(leads)} emails for campaign {campaign_id}", extra={'component': 'server'})
